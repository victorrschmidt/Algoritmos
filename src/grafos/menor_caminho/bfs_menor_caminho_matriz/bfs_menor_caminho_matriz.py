...
def bfs():
    ...
    fila = Queue()  # Fila para verificar cada célula adjacente na matriz
    fila.put([pos_l, pos_c])  # Adicionar a célula inicial na fila ( [eixo X, eixo Y] )
    ...
    while not fila.empty():  # Iteração para cada célula na fila
        l, c = fila.get()  # Pegar a posição Y (linha) da célula e a posição X (coluna) da célula
        ...
        for i in range(4):  # Iteração para cada célula adjacente à célula que está sendo visitada
            ...
            fila.put([ll, cc])  # Adicionar a célula à fila
        ...
        if c_restantes == 0:  # Se foram analisadas todas as células nessa camada de busca
            c_restantes = fila.qsize()  # A próxima camada de busca tem todas as células na próxima camada (tamanho atual da fila)
