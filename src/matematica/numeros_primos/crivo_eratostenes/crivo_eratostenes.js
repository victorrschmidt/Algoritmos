...
function crivo(n) {
    let primos = Array(n+1).fill(true);  // Array de primos com todos os elementos inicializados como true

    primos[0] = primos[1] = false;  // Casos especiais: 0 e 1 não são primos

    for (let p = 2; p <= Math.sqrt(n); p++) {  // Iteração de p no intervalo [2, √n]
        if (!primos[p]) {  // Se o número não é primo
            continue;  // Continuar a iteração
        }

        for (let i = p * p; i <= n; i += p) {  // Iteração de i (múltiplos de p) no intervalo [p * p, n]
